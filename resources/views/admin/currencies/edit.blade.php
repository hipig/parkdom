        ...
        <div class="p-5 lg:p-6 flex-grow w-full">
            <form action="{{ route('admin.settings.currencies.update', $currency) }}" method="post" class="space-y-6">
                ...
                <div class="space-y-1 md:space-y-0 md:flex">
                    ...
                    <input type="text" id="code" name="code" value="{{ old('code', $currency->code) }}" class="block border border-gray-200 rounded px-3 py-2 leading-6 w-full md:w-3/5 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" placeholder="请输入编码">
                </div>
                <div class="space-y-1 md:space-y-0 md:flex">
                    <label for="prefix" class="font-semibold py-2 md:w-1/5 flex-none md:mr-6 text-right">前缀</label>
                    <input type="text" id="prefix" name="prefix" value="{{ old('prefix', $currency->prefix) }}" class="block border border-gray-200 rounded px-3 py-2 leading-6 w-full md:w-3/5 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" placeholder="请输入前缀">
                </div>
                <div class="space-y-1 md:space-y-0 md:flex">
                    ...
                    <div class="w-full md:w-3/5 space-x-6">
                        @foreach(\App\Models\Currency::$statusMap as $key => $value)
                            <label class="inline-flex items-center space-x-2">
                                <input id="status" type="radio" name="status" value="{{ $key }}" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" {{ old('status', $currency->status) == $key ? 'checked' : '' }}>
                                <span>{{ $value }}</span>
                            </label>
                        @endforeach
                    </div>
