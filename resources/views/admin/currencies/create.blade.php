            ...
            <div class="p-5 lg:p-6 flex-grow w-full">
                ...
                <div class="space-y-6 md:w-2/3">
                    <div class="space-y-1">
                        ...
                        <input type="text" id="code" name="code" value="{{ old('code') }}" class="block border border-gray-200 rounded px-3 py-2 leading-6 w-full focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" placeholder="请输入编码">
                    </div>
                    <div class="space-y-1">
                        <label for="prefix" class="text-gray-900 font-semibold"><span class="text-red-600 px-1">*</span>前缀</label>
                        <input type="text" id="prefix" name="prefix" value="{{ old('prefix') }}" class="block border border-gray-200 rounded px-3 py-2 leading-6 w-full focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" placeholder="请输入前缀">
                    </div>
                    <div class="space-y-1">
                        <label for="status" class="text-gray-900 font-semibold"><span class="text-red-600 px-1">*</span>{{ __('Status') }}</label>
                        <div class="flex items-center space-x-6">
                            @foreach(\App\Models\Currency::$statusMap as $key => $value)
                                <label class="inline-flex items-center space-x-2">
                                    <input id="status_{{ $key }}" type="radio" name="status" value="{{ $key }}" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" {{ old('status', \App\Models\Currency::STATUS_ENABLE) == $key ? 'checked' : '' }}>
                                    <span>{{ $value }}</span>
                                </label>
                            @endforeach
                        </div>
