...
@section('title', joinTitle([__('New'), __('Categories')]))
...
@section('content')
    <div class="flex flex-col rounded shadow-sm bg-white overflow-hidden">
        ...
        <form action="{{ route('admin.domainCategories.store') }}" method="post">
            <div class="p-5 lg:p-6 flex-grow w-full">
                ...
                <div class="space-y-6 md:w-2/3">
                    <div class="space-y-1">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}" class="block border border-gray-200 rounded px-3 py-2 leading-6 w-full focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" placeholder="{{ __('Name') }}">
                    </div>
                    <div class="space-y-1">
                        <label for="icon" class="text-gray-900 font-semibold">{{ __('Icon') }}</label>
                        <input type="text" id="icon" name="icon" value="{{ old('icon') }}" class="block border border-gray-200 rounded px-3 py-2 leading-6 w-full focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" placeholder="{{ __('Icon') }}">
                    </div>
                    <div class="space-y-1">
