        ...
        <div class="py-4 px-5 lg:px-6 flex-grow w-full">
            <form action="{{ route('admin.domains.index') }}" method="get" class="space-y-3 sm:space-y-0 sm:flex sm:items-center sm:space-x-4">
                <div class="flex items-center space-x-1">
                    <label for="domain" class="flex-shrink-0 text-sm">域名：</label>
                    <input type="text" id="domain" name="domain" value="{{ old('domain', request()->domain) }}" class="block border border-gray-200 rounded px-3 py-2 leading-5 text-sm w-full focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" placeholder="关键字">
                </div>
                <div class="flex items-center space-x-1">
                    <label for="category_id" class="flex-shrink-0 text-sm">分类：</label>
                    <select id="category_id" name="category_id" class="w-24 block border border-gray-200 rounded px-3 py-2 leading-5 text-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50">
                        <option value="" {{ empty(request()->category_id) ? 'selected' : '' }}>请选择</option>
