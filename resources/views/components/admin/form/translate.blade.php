...
<div x-data="translateContainer">
    <nav class="flex items-center pl-2">
        @foreach(config('app.locales') as $code => $locale)
            <a href="javascript:;" @click="switchTab('{{ $code }}')" class="px-3 md:px-5 text-sm font-medium -mb-px flex items-center space-x-2 py-1.5 border border-b-0 bg-white text-indigo-500 border-gray-200 rounded-t">
                {{ $locale }}
            </a>
        @endforeach
    </nav>
        ...
        @foreach(config('app.locales') as $code => $locale)
            <div x-show="activeTab == '{{ $code }}'">
                ...
                @elseif($type === 'textarea')
                    <textarea id="{{ $name }}_{{ $code }}" name="{{ $name }}[{{ $code }}]" class="w-full block border border-gray-200 rounded px-3 py-2 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" rows="4" placeholder="{{ $placeholder ?? __(\Illuminate\Support\Str::ucfirst($name)) }}">{{ old(join('.', [$name, $code]), $value) }}</textarea>
                @endif
            </div>
        @endforeach
    </div>
