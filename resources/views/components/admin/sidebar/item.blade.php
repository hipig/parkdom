@if($slot->isEmpty())
    <li>
        <a href="{{ $href ?? 'javascript:;' }}"
           class="flex items-center space-x-3 px-3 py-2 hover:bg-gray-800 hover:bg-opacity-75 hover:text-white rounded-t transition ease-out duration-100 rounded-b {{ $active ?? false ? 'bg-gray-800 bg-opacity-75 text-white' : '' }}">
            @if($icon)
